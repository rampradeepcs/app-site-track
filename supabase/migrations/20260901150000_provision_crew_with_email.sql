-- Give a provisioned crew member the address they sign in with.
--
-- provision_company was written before the address became the identity. Its
-- crew insert names phone and not email, and users.email is NOT NULL, so the
-- first crew member aborted the transaction and the founder lost the whole
-- company — the site, the subscription, everything — over a column nobody
-- had asked them for.
--
-- Only the crew loop changes. The founder's own row already carried an
-- address, which is why creating a company with nobody on it would have
-- worked and creating one with a crew would not.
